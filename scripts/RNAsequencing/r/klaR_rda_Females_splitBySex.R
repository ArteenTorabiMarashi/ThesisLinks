...
female_quant_filesIndex <- (grep(pattern = "*_F", quant_files))

female_quant_files <- quant_files[c(female_quant_filesIndex)]
txi <- tximport(female_quant_files, type = "salmon", tx2gene = tx2gene)
...
females_rna.design <- subset(rna.design, rna.design$sex == "female")
females_rna.design <- females_rna.design[,-4]
## For limma Voom
...
design_simple_PCA <- model.matrix(~ lane +
                                    expMatched +
                                    selection,
                                  data = females_rna.design)

dds <- DESeqDataSetFromTximport(txi, females_rna.design,
...
sample_rows <- (grep(pattern = "*F", row_forSamples))
